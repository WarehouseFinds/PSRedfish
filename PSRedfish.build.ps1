...
<#
.SYNOPSIS
    Build script for the 'PSRedfish' PowerShell module

.DESCRIPTION
    This script contains the tasks for building the 'PSRedfish' PowerShell module
#>
[CmdletBinding()]
...
task Export-CommandHelp {
    ...
    $helpFiles = Measure-PlatyPSMarkdown -Path "$buildPath/help/PSRedfish/*.md"
    foreach ($helpFile in $helpFiles) {
        [void] (Test-MarkdownCommandHelp -Path $helpFile.FilePath)
    }

    # Generate module help file
    $mdfiles = Measure-PlatyPSMarkdown -Path "$buildPath/help/PSRedfish/*.md" | Where-Object Filetype -Match 'CommandHelp'
